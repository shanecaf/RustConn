...
- **Closing the tab that owns a split stranded its other sessions on the "Displayed in Split View" placeholder** — when two sessions shared a split (one the tab's owner, another added via Select Tab) and the *owner* tab was closed (rather than closing a pane), the remaining Select-Tabbed session was left showing the "Displayed in Split View" placeholder in its home tab instead of taking over the whole widget. The tab-close cleanup only cleared the closing session from the split bridge; it never returned the split's other (guest) sessions to their home tabs. It now reparents every guest session back to its own tab and resumes its suspended monitoring before the owner tab and its split widget are torn down, so both terminal- and RDP-owner splits recover correctly (the close-pane path already handled this)
- **Embedded RDP on a small window rendered a huge cursor/UI instead of a dense, fully-visible desktop** — on a scaled display (e.g. 2×) a small window was requested at the full display DPI (e.g. 200%), so the server rendered a tiny logical desktop (a ~373×270 logical window at 200%) with the cursor and Windows UI at double size while barely any content fit. The remote desktop mode is now keyed on the widget's *logical* size via the shared `desktop_request_for_area` core helper: when it drops below 640×480, RustConn requests a ≥-minimum desktop scaled up by 2× (or 3× when smaller) at a fixed **100% DPI**, so the server renders a normal-sized cursor and UI, and the embedded viewer downscales that larger frame into the small window — the whole desktop stays visible and small (dense), the cursor is normal-sized, and no reconnect is needed. A comfortably-sized window keeps the full display DPI (HiDPI/retina, #207). Applied on every path that requests a resolution — initial connect, the post-connect settle-snap, debounced resize, and the "Fit resolution" button — so all four share one guard
- **Embedded RDP went blank after closing the other pane of a split (unsplit)** — when a split collapsed back to a single RDP session, the reparented viewer showed nothing (not even its toolbar): the live connection stayed up and frames kept filling the buffer, but the `DrawingArea`'s draw function was never invoked again, so the picture never reached the screen. The cause was re-wrapping the returning RDP widget in a freshly-created `adw::ToastOverlay` during reparent, which left the moved `DrawingArea` unable to repaint. The RDP viewer is now re-parented directly (like the VNC/SPICE viewers), so it fills the tab and renders immediately after unsplit
- **Embedded RDP could enter a resize feedback loop on small windows** — because a small window now requests a 2×/3× desktop, the debounced resize handler's "did the size change" test (which compared the *widget* size to the current *remote resolution*) was always above its threshold, so every layout event re-requested a resolution; each request nudged the widget's allocation by a few pixels, which re-triggered the handler — an endless ping-pong where the window's contents flickered between fitting and not fitting. The handler now compares the *newly computed request* to the current resolution and additionally requires the widget's logical size to have moved beyond a hysteresis (48 px) since the last request, so involuntary few-pixel nudges no longer trigger a resize and the layout settles
- **Clicking on an embedded RDP/VNC panel in split view only changed focus but did not pass mouse events to the remote desktop** — the split panel's capture-phase `GestureClick` walked up the widget tree looking for interactive children (buttons and VTE terminals) and claimed the event for everything else, consuming clicks that should have reached the `DrawingArea`. The handler now also recognises `DrawingArea` and `GLArea` as interactive, denying the gesture so mouse events propagate to the embedded viewer's own input controllers
- **SSH connections via jump host (ProxyCommand) failed with "Permission denied (publickey)" when opened in parallel** — the ProxyCommand ssh did not receive the identity file (`-i`) because it was removed from the shared args to avoid duplication in the outer command, and the code searched those emptied args for `-i`. The resolved identity key is now passed directly to the ProxyCommand, and a `ControlPath` (slave-only) is set so that when the jump host's master socket exists the ProxyCommand reuses it without re-authenticating
- **Workspace restore skipped RDP/VNC/SPICE connections** — opening a saved workspace profile only called the synchronous `start_connection` path, which does not resolve vault credentials for non-SSH protocols. Workspace restore now uses `start_connection_with_credential_resolution` for all entries, so RDP/VNC/SPICE sessions look up their password from the vault and connect correctly
- **Workspace split restore failed for asynchronous connections (RDP/VNC/SPICE)** — when a saved workspace profile included a split with an RDP, VNC, or SPICE session in the second panel, the split was restored empty because `apply_layout` fired before the async credential resolution (vault unlock → SSH tunnel → protocol handshake) completed, so the guest session's tab did not yet exist in the notebook. The workspace profile now persists all guest entries (`split_guests` vec + `extra_splits` count), recreates multi-panel splits during restore, and a deferred placement callback automatically moves each guest session into the next empty panel the moment its tab is created — regardless of how long credential resolution takes. Supports arbitrary panel counts (2, 3, 4+)
- **Workspace restore dropped Local Shell tabs** — Local Shell sessions use `Uuid::nil()` as their connection ID (they are ad-hoc, not backed by a saved connection entry), so the restore loop's `get_connection(nil)` returned `None` and silently skipped them. The restore now detects Local Shell entries by `connection_id.is_nil() && protocol == "local"` and spawns them directly via the same path as the manual "Local Shell" action
- **Multi-panel workspace split restore (3+ panels) applied splits to the wrong tab** — `apply_layout` fired `win.split-*` actions on whatever tab happened to be active at idle time (usually the last-created tab), not the original split owner. Save now records `split_owner_entry_index`, and restore defers split actions until the owner session's tab appears, switches to it, then fires all splits on the correct tab
- **Multi-panel workspace restore lost guest sessions that connected before the owner** — SSH and Local Shell tabs appear synchronously (before RDP/VNC owner tabs that need vault resolution), so `on_tab_added` missed them. Early-arriving guests are now buffered and placed into empty panels via a deferred idle callback after the owner's splits complete
- **Workspace split restore created all panels in one direction** — save only stored a single `horizontal` flag for the root split. Save now captures per-split directions (`split_directions: Vec<bool>`) from the bridge's tree (pre-order DFS), and restore fires the correct `win.split-horizontal` or `win.split-vertical` for each individual split
- **Workspace save recorded duplicate guest indices for multiple Local Shell sessions** — all Local Shell entries share `connection_id = Uuid::nil()`, so `position()` returned the same index for every guest. Save now tracks used entry indices and finds the next unused match for each duplicate connection ID
- **Workspace restore with sync owner (Local Shell) left guest panels empty** — when all sessions are synchronous (Local Shell), `on_tab_added` callbacks interleaved with split idle actions left guests unable to find empty panels. A dedicated idle-deferred placement path now scans the notebook after splits complete and places all sync guests in one pass

### Dependencies

- **Updated**: `cc` 1.2.65→1.2.66, `lzma-rust2` 0.16.4→0.16.5, `num-bigint` 0.4.7→0.4.8 (transitive build/utility crates; no user-facing change)
