...
- **No OBS Debian or Ubuntu package had ever contained the in-tab browser, and the comment explaining why it did was wrong** — `packaging/obs/debian.control` asked for `libwebkitgtk-6.0-dev | libglib2.0-dev`, on the reasoning that apt installs the first branch it can satisfy, so a repository with WebKitGTK 6.0 would opt in and one without would fall back to a package `libgtk-4-dev` already pulls in. OBS does not resolve build dependencies with apt. Its own scheduler picks a branch, prefers the one already in the closure because that costs nothing, and therefore chose `libglib2.0-dev` every time. `debian.rules` then decides the feature with `pkg-config --exists webkitgtk-6.0`, which can only succeed if the dev package is in the chroot — so the probe failed, the feature was compiled out, and the build reported success. Measured in the 0.21.5 build logs for all three deb targets: `libwebkitgtk-6.0-dev` appears zero times, `libglib2.0-dev` twice, and the detection line printed `web:` empty. Because the UI is honest about a build without the feature — it offers only the System and Custom browser modes — the symptom was an option quietly not existing rather than an error, which is why it went unnoticed for two releases. The dependency is now plain, so Debian 13, Ubuntu 24.04 and Ubuntu 26.04 get the embedded browser for the first time. What makes that safe now and unsafe when it was first tried is the repository configuration rather than this file: `xUbuntu_24.04` has since gained an `Ubuntu:24.04/universe` path, which is where Ubuntu keeps WebKitGTK. Checked before changing it, since a wrong answer turns three succeeding builds into failures — 2.52.6 on Ubuntu 26.04, 2.52.6-0ubuntu0.24.04.1 on noble amd64, 2.52.6-1~deb13u1 on trixie amd64, all above the 2.40 floor `webkit6-sys` asks for. A repository that lacks it will now fail and name it, which is the intended outcome; the fix for that case is to gate the feature per repository in the OBS project config, not to restore an alternative that hides it. The RPMs were never affected — the spec has always required `pkgconfig(webkitgtk-6.0)` outright. One note on provenance, since this entry sits under 0.21.5 and the `v0.21.5` tag does not contain it: the change landed on `main` just after the tag was cut, and it reaches users through the OBS packages, which are rebuilt from `main` separately rather than from the tag. Nothing attached to the GitHub release is affected either way — that channel bundles WebKitGTK through `linuxdeploy` for the AppImage and declares it from the binary for the `.deb` and `.rpm`.
