...
- **Updated**: cc 1.4.4→1.4.5, find-msvc-tools 0.1.11→0.1.12, syn 3.0.4→3.0.5, tinyvec 1.12.0→1.13.2, tokio-rustls 0.26.4→0.26.5, zstd-safe 7.2.4→7.3.0, zstd-sys 2.0.16→2.1.0. Thirteen further crates remain behind their latest release because the newer versions are semver-incompatible, so `cargo update` does not reach them.
...
- **OpenH264 is now bundled in the Flathub manifest too, not only the local one** — the embedded RDP client does not link a decoder, it `dlopen`s one at runtime: `gfx_handler.rs` probes `/app/lib/libopenh264.so` and then scans `/app/lib` and `/app/lib64` for a versioned soname. The module supplying it was added to `packaging/flatpak/` alone when the IronRDP GFX handler landed and never to `packaging/flathub/`, so on Flathub the probe found nothing and every AVC420/AVC444 session quietly negotiated down to the RemoteFX path — the one failure mode that code's own comments warn about, on the one build most users install. Both manifests now carry the same openh264 2.6.0 module, and both explain what it is for, since the H.264 module sitting after FreeRDP in the module list reads like a FreeRDP dependency and is not one. Bundling is the only option left: freedesktop dropped `org.freedesktop.Platform.openh264` from the runtime in 2025, so there is no extension to depend on.
