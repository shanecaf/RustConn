...
- **Stay inside the task's scope; when a change escapes it, stop and ask.** If a task ("add argv logging") turns into a refactor, a dedup, a signature change or a cross-type conversion mid-edit, that is a *second* task — split it out and confirm before doing it, do not fold it into the first. On 2026-09-22 an argv-logging change quietly grew into "route the embedded-VNC launcher through the shared core builder"; the two `VncConfig` types (GUI-side `embedded_vnc_types::VncConfig` vs `rustconn_core::VncConfig`) do not convert cleanly, and the detour broke the build and left `.rej`/`.orig` litter. The minimal change — collect the args into a `Vec` for logging, leave the existing builder alone — was both smaller and correct. Opportunistic refactor is exactly the "abstraction nobody asked for" this section forbids, wearing a different hat.
